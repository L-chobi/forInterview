...
- 데이터와 그 데이터에 관련되는 동작(함수)을 모두 포함한 개념
...
### 추상 클래스와 인터페이스의 차이
- 추상 클래스는 extends, 인터페이스는 implements를 사용해서 상속 구현.
- 추상 클래스는 일반 메소드도 가지지만 인터페이스는 추상 메소드만 가질 수 있음.
- 추상 클래스는 비슷한 종류의 클래스들에게 조상의 역할을 한다면, 인터페이스는 다른 조상 클래스들을 가지더라도 공통된 기능을 제공하고 싶을 경우 사용.
: 추상 클래스가 is a(A는 B) 관계라면, 인터페이스는 has a(해당 기능을 가진) 관계.
...
- StringBuffer는 synchronized 키워드가 사용가능한데, 이는 여러 스레드가 하나의 자원에 대해 사용하려고 할 때, 하나의 스레드만 사용하도록 하는 것을 말함. 그렇기에 멀티쓰레드 환경에서 안전하다는 것임.
- StringBuilder는 동기화를 지원하지 않음. 그렇기에 멀티쓰레드에서 안전하지 않지만 반대로 단일쓰레드에서의 성능은 뛰어남.
- StringBuffer, StringBuilder는 가변임. 
